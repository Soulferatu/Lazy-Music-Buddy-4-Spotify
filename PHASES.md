...
**Blocked by (in-flight):** library refactor — see [wiki/library_refactor.md](wiki/library_refactor.md) (top of page has a **Resume Here** section) and [LIBRARY_REFACTOR_PLAN.md](LIBRARY_REFACTOR_PLAN.md). Phases 1+2 and 3 done; Phase 4 (cutover) next, then Phase 5+6 (resolver rewrite). Stage 6 starts after the refactor lands so setlist.fm data uses `data/library/setlists.json` from day one.
