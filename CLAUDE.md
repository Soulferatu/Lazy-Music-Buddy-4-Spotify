...
- **Next:** `py scripts/resolve_lineup.py --retry-unresolved` to recover the 9 improvements observed during the Phase 5+6 cutover run (Craft, Force, Krogi, Maschine, Mr. Hurley, Mantar, Focus, Nergal, E.N.D.). Then Stage 6 (setlist.fm) — the library schema's `setlists.json` slot is ready for it.
